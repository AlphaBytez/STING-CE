...
-- Chatbot user preferences (cross-conversation)
-- NOTE: Renamed from user_preferences to avoid conflict with UI preferences in user_settings table
...
CREATE TRIGGER update_chatbot_preferences_updated_at
    BEFORE UPDATE ON chatbot_preferences
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();
