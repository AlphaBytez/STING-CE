...
-- This script creates all the core tables for the STING application database
-- Note: Database and user creation are handled by subsequent scripts (02-kratos.sql, 03-database-users.sql, 04-messaging-database.sql)
-- 
...
-- App Users table (legacy - mapped to main users table)
...
-- App Sessions table (legacy - mapped to main user_sessions table)
...
-- App Settings table (legacy - mapped to system_settings table)
...
DO $$
BEGIN
    RAISE NOTICE '✅ STING main application tables created successfully!';
    RAISE NOTICE '📊 Created complete schema with preference management';
    RAISE NOTICE '🔑 User authentication and passkey tables initialized';
    RAISE NOTICE '📈 Reporting and analytics infrastructure ready';
    RAISE NOTICE '🤖 Nectar bot and AI assistant support enabled';
    RAISE NOTICE '🏪 Marketplace and honey jar management ready';
    RAISE NOTICE '';
    RAISE NOTICE 'Main application schema initialization complete!';
END $$;