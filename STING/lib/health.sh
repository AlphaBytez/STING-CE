...
check_services_health() {
    ...
    # Additional important services - conditionally include mailpit
    local important_services=("report-worker" "redis")
    if [[ "${EMAIL_MODE}" == "dev" || "${EMAIL_MODE}" == "development" ]] && [[ "${DISABLE_MAILPIT}" != "true" && "${NO_MAILPIT}" != "true" ]]; then
        important_services+=("mailpit")
        log_message "Including mailpit in health checks (EMAIL_MODE: ${EMAIL_MODE})"
    else
        log_message "Skipping mailpit in health checks (Custom SMTP configured)"
    fi
    # Optional services
...
list_all_services_status() {
    ...
    # All known services - conditionally include mailpit
    local all_services=("db" "vault" "kratos" "app" "frontend" "report-worker" 
                       "redis" "chroma" "knowledge" "llm-gateway-proxy" "searxng"
                       "chatbot" "messaging" "external-ai" "utils")
    
    # Add mailpit only if it should be running
    if [[ "${EMAIL_MODE}" == "dev" || "${EMAIL_MODE}" == "development" ]] && [[ "${DISABLE_MAILPIT}" != "true" && "${NO_MAILPIT}" != "true" ]]; then
        all_services+=("mailpit")
    fi
