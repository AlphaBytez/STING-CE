...
show_enhanced_status() {
    ...
    local aux_services=("knowledge" "messaging" "external-ai" "chatbot" "chroma" "redis" "searxng")
    ...
    if timeout 5 curl -sf "${ollama_endpoint}/v1/models" >/dev/null 2>&1; then
        echo "[+] LLM is accessible at ${ollama_endpoint}"
        if [ "$verbose" = "true" ] && command -v ollama >/dev/null 2>&1; then
            local models=$(ollama list 2>/dev/null | tail -n +2 | wc -l)
            echo "   Available models: $models"
        fi
    else
        echo "[!]  LLM endpoint not accessible: ${ollama_endpoint}"
        if [ "$ollama_endpoint" != "http://localhost:11434" ]; then
            echo "   (External LLM configured - verify network connectivity)"
        else
