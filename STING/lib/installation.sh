...
# Fixes IPv6 preference issues and Docker/Tailscale interface conflicts
configure_avahi_for_sting() {
    log_message "Configuring Avahi for optimal network compatibility..." "INFO"

    # Detect primary network interface (usually eth0, ens33, enp0s3, etc.)
    local primary_iface
    ...
    if [ -f /etc/avahi/avahi-daemon.conf ]; then
        sudo cp /etc/avahi/avahi-daemon.conf /etc/avahi/avahi-daemon.conf.bak.$(date +%Y%m%d_%H%M%S)
    fi

    # Create optimized Avahi configuration
    sudo tee /etc/avahi/avahi-daemon.conf > /dev/null << EOF
# STING-optimized Avahi configuration
# Generated by STING installer on $(date)

[server]
# Disable IPv6 to prevent resolution issues in VMware/bridged networks
# Many networks don't properly route IPv6, causing connection failures
use-ipv4=yes
use-ipv6=no

# Only advertise on primary network interface
# Prevents conflicts with Docker (docker0, br-*, veth*) and Tailscale (tailscale0)
allow-interfaces=$primary_iface
deny-interfaces=docker0,br-*,veth*,tailscale0
...
[publish]
# Disable workstation/hinfo publishing (not needed for STING)
publish-hinfo=no
publish-workstation=no

# Ensure addresses and domains are published
...
EOF
    ...
    # Restart Avahi to apply configuration
    log_message "  Restarting Avahi with new configuration..."
...
# Prompt for report generation LLM configuration in CLI mode
prompt_report_llm_configuration() {
    log_message "╔════════════════════════════════════════════════════════════╗"
    log_message "║      📊 Report Generation LLM Configuration                ║"
    log_message "╚════════════════════════════════════════════════════════════╝"
    log_message ""
    ...
    while true; do
        echo ""
        echo "Configure separate LLM endpoint for report generation?"
        echo "  [Y] Yes - Configure a dedicated endpoint for reports"
        echo "  [N] No  - Use the main LLM endpoint for reports (default)"
        echo ""
        read -p "Configure separate endpoint? [y/N]: " configure_report_llm

        case "$configure_report_llm" in
            [Yy]*)
                break
                ;;
            *)
                log_message "Using main LLM endpoint for report generation"
                export REPORT_LLM_ENABLED="false"
                return 0
                ;;
        esac
    done

    export REPORT_LLM_ENABLED="true"
    ...
    # Get report LLM endpoint
    local report_llm_endpoint
    read -p "Report LLM Endpoint [http://localhost:11434]: " report_llm_endpoint
    report_llm_endpoint=${report_llm_endpoint:-http://localhost:11434}
    ...
    while [ "$endpoint_valid" = false ]; do
        ...
        log_message ""
        log_message "Testing report LLM endpoint connectivity..."
        ...
        if curl -sf --max-time 5 "$test_url" >/dev/null 2>&1; then
            ...
            echo "  [R] Retry - Re-enter endpoint URL"
            echo "  [C] Continue anyway - Proceed with installation"
            echo "  [S] Skip - Use main LLM endpoint for reports instead"
            echo ""
            read -p "Select [R/C/S] (default: R): " action
            action=${action:-R}
            ...
            case "$action" in
                [Cc]*)
                    ...
                    ;;
                [Ss]*)
                    log_message "Skipping separate report LLM configuration..."
                    export REPORT_LLM_ENABLED="false"
                    return 0
                    ;;
                *)
                    log_message "Re-enter your endpoint configuration..."
                    read -p "Report LLM Endpoint: " report_llm_endpoint
                    echo ""
                    # Loop continues to re-prompt
                    ;;
            esac
    ...
    export REPORT_LLM_ENDPOINT="$report_llm_endpoint"

    # Model selection for reports - Query real models using OpenAI-compatible API
    log_message ""
    log_message "Fetching available models from report LLM endpoint: $report_llm_endpoint"
    ...
    if available_models=$(curl -s "$report_llm_endpoint/v1/models" 2>/dev/null | jq -r '.data[]?.id' 2>/dev/null | sort); then
        if [ -n "$available_models" ]; then
            ...
            local model_choice
            read -p "Select report generation model [1]: " model_choice
            model_choice=${model_choice:-1}

            if [ "$model_choice" = "$i" ]; then
                read -p "Enter custom model name: " report_model
            elif [ -n "${model_map[$model_choice]}" ]; then
                report_model="${model_map[$model_choice]}"
            else
                log_message "Invalid selection, using recommended: $report_model" "WARNING"
            fi

            # Ask for fallback model
            log_message ""
            log_message "Select fallback model (used if primary model unavailable):"
            read -p "Select fallback model [2]: " fallback_choice
            fallback_choice=${fallback_choice:-2}

            if [ "$fallback_choice" = "$i" ]; then
                read -p "Enter custom fallback model name: " fallback_model
            elif [ -n "${model_map[$fallback_choice]}" ]; then
    ...
    export REPORT_LLM_MODEL="$report_model"
    export REPORT_LLM_FALLBACK_MODEL="$fallback_model"

    # Ask for max tokens (with smart defaults)
    log_message ""
    log_message "Report Generation Token Limits"
    log_message "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    log_message ""
    ...
    local max_tokens
    read -p "Maximum output tokens for reports [16384]: " max_tokens
    ...
    log_message "   Max tokens: $max_tokens"
    log_message ""
}
...
update_config_yml_from_cli() {
    ...
    # Update report generation LLM configuration if provided
    if [ "${REPORT_LLM_ENABLED:-false}" = "true" ] && [ -n "${REPORT_LLM_ENDPOINT:-}" ]; then
        log_message "  Setting report generation LLM endpoint: ${REPORT_LLM_ENDPOINT}"

        # Use yq if available for more precise YAML editing, otherwise use sed
        if command -v yq >/dev/null 2>&1; then
            # Use yq for precise YAML manipulation
            yq -i ".llm_service.report_generation.endpoint = \"${REPORT_LLM_ENDPOINT}\"" "$config_file" 2>/dev/null || true
            yq -i ".llm_service.report_generation.model = \"${REPORT_LLM_MODEL}\"" "$config_file" 2>/dev/null || true
            yq -i ".llm_service.report_generation.fallback_model = \"${REPORT_LLM_FALLBACK_MODEL}\"" "$config_file" 2>/dev/null || true
            yq -i ".llm_service.report_generation.max_tokens = ${REPORT_MAX_TOKENS}" "$config_file" 2>/dev/null || true
        else
            # Fallback to sed-based approach
            # Add endpoint field if it doesn't exist in report_generation section
            if ! grep -q "endpoint:" "$config_file" | grep -A 5 "report_generation:"; then
                # Insert endpoint after report_generation: line
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "/report_generation:/a\\
    endpoint: \"${REPORT_LLM_ENDPOINT}\"
" "$config_file"
                else
                    sed -i "/report_generation:/a\\    endpoint: \"${REPORT_LLM_ENDPOINT}\"" "$config_file"
                fi
            fi

            # Update model, fallback_model, and max_tokens in report_generation section
            if [[ "$OSTYPE" == "darwin"* ]]; then
                sed -i '' "/report_generation:/,/^  [a-z_]*:/ s|model: \".*\"|model: \"${REPORT_LLM_MODEL}\"|" "$config_file"
                sed -i '' "/report_generation:/,/^  [a-z_]*:/ s|fallback_model: \".*\"|fallback_model: \"${REPORT_LLM_FALLBACK_MODEL}\"|" "$config_file"
                sed -i '' "/report_generation:/,/^  [a-z_]*:/ s|max_tokens: [0-9]*|max_tokens: ${REPORT_MAX_TOKENS}|" "$config_file"
            else
                sed -i "/report_generation:/,/^  [a-z_]*:/ s|model: \".*\"|model: \"${REPORT_LLM_MODEL}\"|" "$config_file"
                sed -i "/report_generation:/,/^  [a-z_]*:/ s|fallback_model: \".*\"|fallback_model: \"${REPORT_LLM_FALLBACK_MODEL}\"|" "$config_file"
                sed -i "/report_generation:/,/^  [a-z_]*:/ s|max_tokens: [0-9]*|max_tokens: ${REPORT_MAX_TOKENS}|" "$config_file"
...
force_cleanup_docker_resources() {
    ...
    # NOTE: For fresh installs, ALL volumes must be removed including postgres_data
    # Otherwise, PostgreSQL will skip init scripts and tables won't be created!
    for vol in $(docker volume ls -q | grep -E "(sting|kratos|chroma|knowledge|postgres)"); do
        if [[ "$purge_all" == "true" ]]; then
            # Fresh install: Remove ALL volumes including data volumes
            log_message "Removing volume: $vol (purge-all mode)"
            docker volume rm -f "$vol" 2>/dev/null || true
        elif [[ ! "$vol" =~ (postgres_data|vault-data|redis_data|chroma_data|mailpit_data) ]]; then
            # Upgrade mode: Only remove non-data volumes
            docker volume rm -f "$vol" 2>/dev/null || true
        else
            log_message "Preserving data volume: $vol (use --purge-all for fresh install)" "WARNING"
        fi
...
##        "supertokens_logs"  # DEPRECATED  # DEPRECATED
    ...
    if docker ps --format "{{.Names}}" | grep -q -E "(sting|kratos|chroma|knowledge)"; then
        ...
        # Pass purge flag to force_cleanup to remove postgres_data too
        if [ "$purge_models" = "--purge" ]; then
            force_cleanup_docker_resources "true"
        else
            force_cleanup_docker_resources "false"
        fi
    fi
...
uninstall_msting_with_confirmation() {
    ...
    echo ""
    echo "   Add --purge-all for a TRUE FRESH INSTALL including:"
    echo "     - All of the above PLUS database volumes"
    echo "     - This ensures init scripts run on next install"
    echo ""
...
configure_hostname() {
    ...
    # Update existing kratos.yml with hostname (preserving other ${VAR} substitutions)
    # NOTE: Don't regenerate - config_loader.py already handled all variable substitutions
    if [ -f "${INSTALL_DIR}/kratos/kratos.yml" ]; then
        log_message "Updating kratos.yml with hostname: $STING_HOSTNAME"
        if [[ "$(uname)" == "Darwin" ]]; then
            sed -i '' "s|__STING_HOSTNAME__|${STING_HOSTNAME}|g" "${INSTALL_DIR}/kratos/kratos.yml"
        else
            sed -i "s|__STING_HOSTNAME__|${STING_HOSTNAME}|g" "${INSTALL_DIR}/kratos/kratos.yml"
        fi
        log_message "[+] Updated kratos.yml with hostname: $STING_HOSTNAME"
    else
        log_message "[!] kratos.yml not found - config generation may have failed" "WARNING"
    fi
