...
clear_docker_cache() {
    ...
    local skip_container_stop="${3:-false}"  # New: skip stopping containers (for build-first pattern)
    ...
    case "$clear_level" in
        "service")
            ...
            # Only stop and remove container if not skipped (build-first pattern)
            if [ "$skip_container_stop" = "false" ]; then
                ...
                # Only remove image when container is stopped (safe to remove)
                docker images --format '{{.Repository}}:{{.Tag}}' | grep "^sting-ce-${service_name}:latest$" | xargs -r docker rmi -f
            else
                echo -e "${YELLOW}Skipping container stop and image removal (build-first pattern - container keeps running during build)${NC}"
                echo -e "${YELLOW}Old image will be replaced after successful build${NC}"
                # DON'T remove image here - it's needed by running container
                # The new build will create a new image, and docker compose up will swap it
            fi

            # Clear build cache only (not images)
...
build_docker_services_nocache() {
    ...
    local skip_container_stop="${3:-false}"  # Skip stopping containers (build-first pattern)
    ...
    if [ "$cache_level" != "skip" ]; then
        ...
        if [ "$cache_level" = "frontend-extreme" ]; then
            ...
            clear_docker_cache "service" "$service" "$skip_container_stop"
