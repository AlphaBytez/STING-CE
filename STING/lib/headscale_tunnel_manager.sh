...
create_support_tunnel() {
    ...
    log_message "  🔑 Auth Key: ${auth_key:0:4}****${auth_key: -4}"
    ...
    log_message "  2. Join network: tailscale up --login-server=$HEADSCALE_URL --authkey=<provided-key>"
    ...
    log_message ""
    log_message "  \ud83d\udcc4 Full auth key saved to: ${TUNNEL_SESSIONS_DIR}/${ticket_id}.json"
