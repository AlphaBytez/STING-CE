...
generate_initial_configuration() {
    ...
    # Priority: 1. Wizard config (ALWAYS use if present, overwrite existing), 2. Existing config, 3. Default template
    if [ -f "/tmp/sting-setup-state/config.yml" ]; then
        cp "/tmp/sting-setup-state/config.yml" "${CONFIG_DIR}/config.yml"
        log_message "[+] Using wizard-configured config.yml (overwriting any existing)"
    ...
    if [ -f "${CONFIG_DIR}/config_loader.py" ]; then
        ...
        if exec_in_utils "cd /app/conf && INSTALL_DIR=/app python3 config_loader.py config.yml --mode $mode"; then
            ...
            if [ "${normalized_source}" = "${normalized_install}" ]; then
                ...
                if cp "${SOURCE_DIR}/env/"*.env "${INSTALL_DIR}/env/" 2>/dev/null || sudo cp "${SOURCE_DIR}/env/"*.env "${INSTALL_DIR}/env/" 2>/dev/null; then
                    # Fix ownership after copy
                    sudo chown -R "$USER:$(id -gn)" "${INSTALL_DIR}/env/"*.env 2>/dev/null || true
...
generate_kratos_config() {
    ...
    # Generate to ${INSTALL_DIR}/kratos/ where docker-compose expects it
    local kratos_conf_dir="${INSTALL_DIR}/kratos"
    local output_file="$kratos_conf_dir/kratos.yml"
    mkdir -p "$kratos_conf_dir"
    
    # Source email.env if it exists to get SMTP configuration from wizard
    ...
    # Determine SMTP configuration based on EMAIL_MODE
    local smtp_uri
    local smtp_from_address
    ...
    if [[ "$email_mode" == "production" ]]; then
        ...
        # Development mode - use mailpit with platform-specific config
        if [[ "$(uname)" == "Darwin" ]]; then
            ...
            log_message "Detected macOS - using Mac-specific mailpit configuration"
        else
    ...
    # Allow explicit environment variable override (highest priority)
...
selfservice:
  ...
  flows:
    error:
      ui_url: ${FRONTEND_URL}/error
    settings:
      ui_url: ${FRONTEND_URL}/settings
      privileged_session_max_age: ${aal2_timeout}
      required_aal: highest_available  # Kratos v1.3.0+ only accepts 'aal1' or 'highest_available'
    login:
            ...
            config:
              ...
              auth:
                ...
                config:
                  ...
                  value: "${KRATOS_WEBHOOK_TOKEN:-webhook-token-change-me}"
        default_browser_return_url: ${FRONTEND_URL}/dashboard
    recovery:
      enabled: true
    ...
    logout:
      after:
        default_browser_return_url: ${FRONTEND_URL}/login
  methods:
    password:
      enabled: false  # Passwordless auth - passwords disabled
      config:
    ...
    code:
      enabled: true
      passwordless_enabled: true  # Enable passwordless email code flow
      config:
        lifespan: 15m
...
session:
  ...
  whoami:
    required_aal: aal1  # Allow AAL1 sessions for whoami (AAL2 enforced at app level during step-up)
