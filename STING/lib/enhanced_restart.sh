...
restart_with_dependency_order() {
    ...
    # Sync Vault token to env files after Vault starts
    # This ensures subsequent services get the correct token
    if type sync_vault_token_to_env &>/dev/null; then
        sync_vault_token_to_env
    fi

    # Tier 2: Authentication and messaging (depends on Tier 1)  
    # Check if mailpit should be started based on EMAIL_MODE and user preferences
    local auth_services="kratos messaging"
    if [[ "${EMAIL_MODE}" == "dev" || "${EMAIL_MODE}" == "development" ]] && [[ "${DISABLE_MAILPIT}" != "true" && "${NO_MAILPIT}" != "true" ]]; then
        auth_services="kratos mailpit messaging"
        log_message "Including mailpit in authentication tier (EMAIL_MODE: ${EMAIL_MODE})"
    else
        log_message "Skipping mailpit in authentication tier (EMAIL_MODE: ${EMAIL_MODE}, Custom SMTP configured)"
    fi
    
    if ! start_service_tier "Authentication" $auth_services; then
...
start_service_tier() {
    ...
    # Get list of services defined in compose file (not just running ones)
    local compose_services=$(docker compose config --services 2>/dev/null)
    ...
    for service in "${services[@]}"; do
        if echo "$compose_services" | grep -q "^${service}$"; then
            ...
            log_message "Service $service not defined in compose, skipping..." "WARNING"
        fi
    done
