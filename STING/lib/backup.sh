...
rotate_backups() {
    ...
    local max_age_days="${3:-$BACKUP_MAX_AGE_DAYS}"
    ...
    log_message "Rotating backups in $backup_dir (keeping $keep_count most recent, max age: ${max_age_days} days)"

    if [[ "$(uname)" == "Darwin" ]]; then
        # macOS version using BSD find and stat
        cd "${backup_dir}" || return 1

        # Age-based cleanup first (remove backups older than max_age_days)
        if [ -n "$max_age_days" ] && [ "$max_age_days" -gt 0 ]; then
            local age_deleted=$(find . -name '*.tar.gz' -mtime +${max_age_days} -type f 2>/dev/null)
            if [ -n "$age_deleted" ]; then
                echo "$age_deleted" | xargs rm -f
                log_message "Removed $(echo "$age_deleted" | wc -l) backups older than ${max_age_days} days"
            fi
        fi

        # Then count-based cleanup (keep only keep_count most recent)
        local remaining=$(ls -t *.tar.gz 2>/dev/null | tail -n +$((keep_count + 1)) | wc -l)
        if [ "$remaining" -gt 0 ]; then
            ls -t *.tar.gz 2>/dev/null | tail -n +$((keep_count + 1)) | xargs -I {} rm -f {}
            log_message "Removed $((remaining)) excess backups (keeping $keep_count most recent)"
        fi
    else
        # Linux version using GNU find
        # Age-based cleanup first (remove backups older than max_age_days)
        if [ -n "$max_age_days" ] && [ "$max_age_days" -gt 0 ]; then
            local age_count=$(find "${backup_dir}" -name '*.tar.gz' -mtime +${max_age_days} -type f 2>/dev/null | wc -l)
            if [ "$age_count" -gt 0 ]; then
                find "${backup_dir}" -name '*.tar.gz' -mtime +${max_age_days} -type f -delete
                log_message "Removed $age_count backups older than ${max_age_days} days"
            fi
        fi
        ...
        local remaining=$(find "${backup_dir}" -name '*.tar.gz' -type f 2>/dev/null | wc -l)
        log_message "Backup rotation complete. Total backups remaining: $remaining"
    fi
}
