...
## ⚠️ CRITICAL: Backup Encryption Keys Before Upgrade

**Before ANY upgrade or migration**, backup your encryption keys:

```bash
# Backup encryption keys (CRITICAL - do this first!)
...
**Why this matters:**
- The Honey Reserve Master Key encrypts all user files (profile pictures, attachments, etc.)
- If this key is lost or changed, ALL encrypted files become **permanently unreadable**
- The system will now REFUSE to auto-generate new keys to prevent data loss

See [Encryption Key Management](#encryption-key-management) for more details.
...
STING-CE uses the **Honey Reserve Master Key** to encrypt:
- Profile pictures
...
- Stored securely in Vault at `sting/honey_reserve`
- Used to derive per-user encryption keys via HKDF

**⚠️ CRITICAL**: Loss of this key = permanent loss of ALL encrypted user data!
...
# Backup keys (do this before upgrades!)
msting encryption-keys backup
# Creates: sting-encryption-keys.backup

# Backup to specific file
msting encryption-keys backup /secure/path/my-keys.backup

# Restore keys from backup
msting encryption-keys restore /path/to/backup-file
```
...
1. **Backup Before Upgrades**
   ```bash
   msting encryption-keys backup
   # Then store backup securely BEFORE running upgrade
   ```

2. **Multiple Backup Locations**
   - Password manager (1Password, Bitwarden, etc.)
   - Encrypted USB drive in secure location
   - Encrypted cloud storage (distinct from your STING data)

3. **Test Your Backups**
   - Periodically verify you can read the backup file
   - Test restore in a non-production environment

4. **Never Share Keys**
   - Treat like root passwords
   - Don't commit to git
   - Don't send via email

### Recovery Scenarios

#### Scenario: Key was accidentally regenerated

If you upgraded and profile pictures stopped working:

```bash
# Check if you have old key in env file
grep HONEY_RESERVE_MASTER_KEY /opt/sting-ce/env/app.env

# If found, restore it to Vault
msting encryption-keys restore /path/to/backup

# Or manually update Vault
docker exec sting-ce-vault vault kv put sting/honey_reserve master_key="YOUR_OLD_KEY_HERE"
...
Unfortunately, if the key is truly lost:
- Existing encrypted files **cannot be recovered**
- Users will need to re-upload profile pictures and files
- Contact support for data recovery options (limited)

This is why we now **refuse to auto-generate new keys** - it's better to fail loudly than silently corrupt data.
