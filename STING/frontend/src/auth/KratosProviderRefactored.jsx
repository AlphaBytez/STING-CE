...
export const KratosProviderRefactored = ({ children }) => {
  ...
  // DISABLED: This effect was causing flickering loops on mobile after new logins
  // The initial checkSession() on mount should be sufficient
  // If session issues persist, the user can manually refresh
  /*
  // Force session refresh when we detect recent authentication
  useEffect(() => {
    // CRITICAL: Skip this effect if we already have a session
    // This prevents the loading loop on mobile
    if (session || identity) {
      // Already authenticated, clear markers and skip
      sessionStorage.removeItem('sting_recent_auth');
      sessionStorage.removeItem('sting_auth_processing');
  ...
  }, [session, isLoading, identity]); // Added identity to dependencies
  */
  
  // Clear any stale auth markers on mount to prevent issues
  useEffect(() => {
    // Clean up any stale markers from previous sessions
    const recentAuth = sessionStorage.getItem('sting_recent_auth');
    if (recentAuth) {
      const authTime = parseInt(recentAuth);
      // If marker is older than 30 seconds, it's stale - remove it
      if (Date.now() - authTime > 30000) {
        sessionStorage.removeItem('sting_recent_auth');
        sessionStorage.removeItem('sting_auth_processing');
        sessionStorage.removeItem('sting_auth_retry_count');
      }
