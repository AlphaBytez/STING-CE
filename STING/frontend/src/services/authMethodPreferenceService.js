...
class AuthMethodPreferenceService {
   ...
   */
  detectServiceUpdate() {
    // Check if WebAuthn is currently unreliable (set by server during updates)
    // SECURITY: Only check sessionStorage (browser-session scoped, not persistent)
    return sessionStorage.getItem('webauthn_unreliable') === 'true';
  }
