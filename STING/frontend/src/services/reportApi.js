...
// Timeout constants - increased for reliability with auth middleware
const TEMPLATES_TIMEOUT = 15000;  // 15 seconds for templates (auth + db query)
const REPORTS_TIMEOUT = 30000;    // 30 seconds for reports list (can be larger dataset)
const REPORT_DETAIL_TIMEOUT = 20000; // 20 seconds for single report details
...
class ReportApiService {
    ...
    async getTemplates(category = null) {
        try {
            ...
            // Handle timeout errors gracefully - return empty templates instead of crashing
            if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
                console.warn('Templates request timed out - server may be busy');
                return {
                    success: false,
                    error: 'Request timed out - please try again',
                    data: {
                        templates: this.getDemoTemplates()  // Fall back to demo templates
                    }
                };
            }
            throw error;
    ...
    async listReports(params = {}) {
        try {
            ...
            if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
                console.warn('Reports request timed out - server may be busy');
                return {
                    success: false,
                    error: 'Request timed out - please try again',
                    data: {
                        reports: this.getDemoReports(),  // Fall back to demo reports
                        pagination: { total: 0 }
                    }
                };
            }
    ...
    // Get report system configuration (ReviewBee status, features, etc.)
    async getConfig() {
        try {
            const response = await apiClient.get(`${REPORT_BASE_URL}/public/config`);
            return response.data;
        } catch (error) {
