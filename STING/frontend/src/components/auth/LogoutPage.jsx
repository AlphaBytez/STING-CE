  ...
  useEffect(() => {
    ...
    const processLogout = async () => {
      try {
        ...
        // Clear session-related localStorage items while preserving user preferences
        // SECURITY: Clear all auth/session data, keep only UI preferences
        const preserveKeys = [
          'sting-theme',
          'sting-performance-mode', 
          'sting-chat-mode',
          'language',
          'honeyJarStats_collapsed'
        ];
        const preserved = {};
        ...
        // Even if backend logout fails, clear local storage (preserve preferences)
        const preserveKeys = ['sting-theme', 'sting-performance-mode', 'sting-chat-mode', 'language', 'honeyJarStats_collapsed'];
        const preserved = {};
        preserveKeys.forEach(key => {
          const val = localStorage.getItem(key);
          if (val !== null) preserved[key] = val;
