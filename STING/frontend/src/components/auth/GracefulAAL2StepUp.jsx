...
const GracefulAAL2StepUp = () => {
  ...
  const aal2Failed = searchParams.get('aal2_failed') === 'true';
  
  // Check if this is post-login security upgrade (newuser flow)
  const isNewUserFlow = searchParams.get('newuser') === 'true';
  const reason = searchParams.get('reason') || 'Access protected resource';
  ...
  const [availableMethods, setAvailableMethods] = useState({ webauthn: false, totp: false });
  ...
  useEffect(() => {
    ...
    if (userAAL === 'aal2') {
      ...
      // Auto-redirect to return URL after showing success
      setTimeout(() => {
        navigate(returnTo);
      }, 1500);
    } else {
    ...
    // Check configured credentials from Kratos session
    const credentials = session?.identity?.credentials || {};
    const hasWebAuthn = !!credentials.webauthn;
    const hasTotp = !!credentials.totp;
    
    console.log('🔍 User credentials:', { hasWebAuthn, hasTotp, credentials: Object.keys(credentials) });
    setAvailableMethods({ webauthn: hasWebAuthn, totp: hasTotp });
  ...
  // Check if user has NO 2FA methods set up
  const hasNo2FA = !loadingMethods && !availableMethods.webauthn && !availableMethods.totp;
