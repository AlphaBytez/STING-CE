...
const AuthFlowRouterInner = ({ mode = 'login' }) => {
  ...
  const [currentFlow, setCurrentFlow] = useState('email'); // email, passkey, totp
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        if (isAAL2) {
          // Redirect to security-upgrade which has working credential detection
          console.log('🔐 AAL2 detected in URL - redirecting to security-upgrade');
          const returnTo = new URLSearchParams(window.location.search).get('return_to') || '/dashboard';
          window.location.href = `/security-upgrade?return_to=${encodeURIComponent(returnTo)}`;
          return;
        }
  ...
  const handleCancel = () => {
    // Always go back to dashboard for cancel (since AAL2 now redirects to security-upgrade)
    navigate('/dashboard');
  };
  ...
  const renderCurrentFlow = () => {
    switch (currentFlow) {
      ...
      // AAL2 flows are handled by redirect to security-upgrade
      default:
        return (
