  ...
  const handleCodeSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 422 || response.isAAL2Required) {
        ...
        // Check error type: browser_location_change_required means AAL2 step-up needed
        const errorId = response.data?.error?.id;
        if (errorId === 'browser_location_change_required') {
          console.log('🔐 Kratos requires AAL2 step-up (browser_location_change_required)');
          const redirectUrl = response.data.redirect_browser_to;
          
          // IMPORTANT: Do NOT follow Kratos redirect directly - it will lose session cookies
          // and fail with "aal2 can only be requested if a session exists"
          // Instead, navigate to our security-upgrade page which handles AAL2 properly
          if (redirectUrl && redirectUrl.includes('aal=aal2')) {
            console.log('🔐 AAL2 redirect detected, navigating to security-upgrade instead of:', redirectUrl);
