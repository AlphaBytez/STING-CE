...
// Utility to convert absolute URL to relative path for proxy routing
const cleanActionUrl = (url) => url?.startsWith('http') ? url.replace(/https?:\/\/[^\/]+/, '') : url;
...
const PasskeyManagerDirect = ({ isEnrollmentMode = false, onSetupComplete = null }) => {
  ...
  const check2FAStatus = async () => {
    try {
      ...
      // SECURITY: Only use sessionStorage for recent setup (cleared on browser close)
      // Do NOT trust localStorage as it can be manipulated
      const recentTOTPSetup = sessionStorage.getItem('totp_recently_setup') === 'true';
      
      const hasTOTP = credentialTOTP || sessionTOTP || recentTOTPSetup;
  ...
  const startRegistration = async () => {
    ...
    try {
      ...
      let actionUrl = cleanActionUrl(flow.ui.action);
      ...
      if (!window.oryWebAuthnRegistration) {
        ...
        if (scriptNode?.attributes?.src) {
          const scriptUrl = cleanActionUrl(scriptNode.attributes.src);
  ...
  const removePasskey = async (passkeyId) => {
    ...
    try {
      ...
      await axios.post(cleanActionUrl(flow.ui.action), formData, {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
          'Accept': 'application/json'
        },
