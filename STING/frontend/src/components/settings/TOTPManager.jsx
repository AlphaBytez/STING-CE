...
import { Smartphone, AlertCircle, CheckCircle, X, Info, Loader, Copy } from 'lucide-react';
...
const TOTPManager = ({ isEnrollmentMode = false, onSetupComplete = null }) => {
  ...
  const triggerTOTPSetup = async (flow) => {
    try {
      ...
      if (!setupNode) {
        ...
        // Convert absolute URL to relative path for nginx proxy routing
        const actionUrl = flow.ui.action.startsWith('http')
            ? flow.ui.action.replace(/https?:\/\/[^\/]+/, '')
            : flow.ui.action;

        console.log('🔐 TOTP Setup: Posting to URL:', actionUrl);
  ...
  const verifyTOTP = async () => {
    ...
    try {
      ...
      // Convert absolute URL to relative path for nginx proxy routing
      const actionUrl = settingsFlow.ui.action.startsWith('http')
          ? settingsFlow.ui.action.replace(/https?:\/\/[^\/]+/, '')
          : settingsFlow.ui.action;
      ...
      if (response.status === 200 || response.status === 303) {
        ...
        // Mark TOTP as recently set up for passkey manager (sessionStorage only - clears on browser close)
        // SECURITY: Do NOT use localStorage - it's exploitable
