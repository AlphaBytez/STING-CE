...
import { User, Camera, Upload, Save, X, Loader2 } from 'lucide-react';
...
const ProfileSettings = () => {
  ...
  const { profileData, updateProfileData, profilePicture, uploadProfilePicture, profilePictureLoading } = useProfile();
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        // Create fallback profile data from available sources (no profilePicture - that comes from Vault)
        ...
        // Profile picture comes from Vault via ProfileContext, not from backend API
        ...
        setPreviewImage(profilePicture); // Use Vault picture
      }
    };
    
    fetchProfile();
  }, [profilePicture]); // Re-run when profilePicture loads from Vault
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      // Upload profile picture to Vault if a new one was selected
      if (pendingFile) {
        setIsUploadingPicture(true);
        const uploadResult = await uploadProfilePicture(pendingFile);
        if (!uploadResult.success) {
          alert(`Failed to upload profile picture: ${uploadResult.error}`);
          // Continue with other profile updates even if picture fails
        } else {
          console.log('Profile picture uploaded to Vault successfully');
          setPendingFile(null);
        }
        setIsUploadingPicture(false);
      }
      
      // Profile payload WITHOUT profilePicture (it's stored in Vault, not DB)
      ...
      // Update the profile context (without picture - that's separate)
  ...
  const handleCancel = () => {
    ...
    setPreviewImage(profilePicture); // Reset to current Vault picture
    setPendingFile(null); // Clear any pending upload
    setIsEditing(false);
  };
