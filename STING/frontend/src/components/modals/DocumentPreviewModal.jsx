...
const DocumentPreviewModal = ({ isOpen, onClose, document: docData, honeyJarId }) => {
  ...
  useEffect(() => {
    if (isOpen && docData && viewMode === 'preview') {
      loadPreview();
    }
  }, [isOpen, docData, viewMode]);

  const loadPreview = async () => {
    if (!docData || !honeyJarId) return;
    ...
    try {
      const response = await fetch(
        `/api/knowledge/honey-jars/${honeyJarId}/documents/${docData.id}/preview`,
        {
          credentials: 'include',
          headers: {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch(
        `/api/knowledge/honey-jars/${honeyJarId}/documents/${docData.id}/download`,
        {
          credentials: 'include',
        }
      );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">
                {formatBytes(docData?.size_bytes)} • {docData?.content_type || 'Unknown type'}
              </p>
            </div>
          </div>
          <button
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-white font-medium">{docData?.filename}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-400 mb-1">File Size</p>
                  <p className="text-white font-medium">{formatBytes(docData?.size_bytes)}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-400 mb-1">Content Type</p>
                  <p className="text-white font-medium">{docData?.content_type || 'Unknown'}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-400 mb-1">Upload Date</p>
                  <p className="text-white font-medium">{formatDate(docData?.upload_date)}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-white font-medium">{docData?.embedding_count || 0}</p>
                </div>
              </div>
