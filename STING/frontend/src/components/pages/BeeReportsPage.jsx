...
import { Button, Table, Tag, Progress, Input, Select, Tooltip, Badge, Space, message, Tabs } from 'antd';
...
const BeeReportsPage = () => {
  ...
  // ReviewBee and system config state
  const [systemConfig, setSystemConfig] = useState({
    review_bee: { enabled: false },
    features: {}
  });
  ...
  useEffect(() => {
    const loadConfig = async () => {
      try {
        const response = await reportApi.getConfig();
  ...
  // Set up smart polling for active reports
  // - Fast polling (3s) when reports are actively processing
  // - Slower polling (10s) when reports are only queued (waiting)
  // - No polling when no active reports
  useEffect(() => {
    ...
    const hasProcessing = newReportQueue.some(r => r.status === 'processing');
    const hasQueued = newReportQueue.some(r => ['pending', 'queued'].includes(r.status));
    
    // Determine polling interval based on report states
    // Processing = fast updates (3s), Queued only = slower (10s)
    const targetInterval = hasProcessing ? 3000 : (hasQueued ? 10000 : null);
    
    // Clear existing interval if target changed
    if (pollInterval) {
    ...
    if (targetInterval) {
      const interval = setInterval(() => {
        // Only fetch reports, queue status is updated with reports
  ...
  }, [userReports]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    },
    {
      ...
      render: (status, record) => (
        <div className="flex flex-col gap-1">
          ...
          {status === 'processing' && record.status_message && (
            ...
            <div className="dashboard-card p-6 h-full rounded-xl">
              ...
              {reportQueue.length === 0 ? (
                <div className="text-center py-8">
                  <CheckCircleOutlined className="text-6xl text-gray-600 mb-4" />
                  ...
                  {reportQueue.map((report) => (
                    <div key={report.id} className="stats-card p-4">
                      ...
                      <p className="text-gray-400 text-xs mb-3">
                        {report.status_message || report.description || 'Processing...'}
                      </p>
                      {report.status === 'processing' && (
                        <div className="space-y-3">
                          {/* Stage Progress Indicator */}
                          <div className="flex items-center gap-1 text-xs">
                            {['collecting', 'generating', 'reviewing', 'formatting', 'saving'].map((stage, idx) => {
                              const stages = ['collecting', 'generating', 'reviewing', 'formatting', 'saving'];
                              const currentIdx = stages.indexOf(report.current_stage || 'collecting');
                              ...
                              return (
                                <div key={stage} className="flex items-center gap-1">
                                  ...
                                  <span className={`
                                    ...
                                    ${isCurrent ? 'text-yellow-400 font-medium' : ''}
                                    ${isPending ? 'text-gray-500' : ''}
                                  `}>
                                    {stage === 'collecting' && 'Collect'}
                                    {stage === 'generating' && 'Generate'}
                                    {stage === 'reviewing' && 'Review'}
                                    {stage === 'formatting' && 'Format'}
                                    {stage === 'saving' && 'Save'}
                                  </span>
                                  {idx < stages.length - 1 && (
                                    <span className={`mx-1 ${isComplete ? 'text-green-500' : 'text-gray-600'}`}>→</span>
                                  )}
                                </div>
                              );
                            })}
            ...
            <div className="dashboard-card p-6 h-full rounded-xl">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <PlusOutlined className="text-yellow-400 text-xl" />
                  ...
                  <Badge 
                    ...
                    title={`${filteredAvailableTemplates.length} templates available`}
                  />
                </div>
                <div className="flex items-center gap-3">
                  <Search
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-white">Report Bee</h1>
          </div>
          
          {/* AI Quality Assurance Indicator */}
          {systemConfig.review_bee?.enabled && (
            <Tooltip title="Reports are automatically reviewed for accuracy and quality before delivery">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/40">
                <CheckCircleOutlined className="text-green-400" />
                <span className="text-green-400 text-sm font-medium">AI Quality Check</span>
              </div>
            </Tooltip>
          )}
        </div>
        <p className="text-gray-400">
          Generate comprehensive security and analytics reports from your STING data
        </p>
      </div>

      {/* Quick tip */}
      {queueStatus && queueStatus.currently_processing > 0 && (
        <div className="sting-glass-subtle border border-yellow-500/30 rounded-lg p-3 mb-6">
          <div className="flex items-center gap-2">
            <LoadingOutlined className="text-yellow-400" />
            <span className="text-yellow-200/80 text-sm">
              {queueStatus.currently_processing} report{queueStatus.currently_processing > 1 ? 's' : ''} currently being generated
              {queueStatus.total_pending > 0 && ` • ${queueStatus.total_pending} in queue`}
            </span>
          </div>
        </div>
      )}
