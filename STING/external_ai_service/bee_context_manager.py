    ...
    async def load_brain_knowledge(self) -> str:
        ...
        try:
            if self.brain_path.exists():
                ...
                logger.info(f"Loaded Bee Brain knowledge: {len(self.brain_knowledge)} characters from {self.brain_path}")
            else:
                logger.warning(f"Bee Brain file not found at {self.brain_path}")
