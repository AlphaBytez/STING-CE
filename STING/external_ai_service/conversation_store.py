...
class PostgresConversationStore(ConversationStore):
        ...
        try:
            ...
            if is_new_conversation:
                ...
                async with self._pg_pool.acquire() as conn:
                    await conn.execute(
                        """
                        INSERT INTO conversations (id, user_id, conversation_type, title)
                        VALUES ($1, $2, 'bee_chat', $3)
            ...
            elif conv and not conv.get("title") and role == "user" and content:
    ...
    async def list_conversations(
        ...
        async with self._pg_pool.acquire() as conn:
            rows = await conn.fetch(
                """
                SELECT c.id, c.title, c.conversation_type, c.status, c.is_pinned,
