...
CREATE TABLE IF NOT EXISTS conversations (
    ...
    bot_id VARCHAR(255) DEFAULT 'bee',  -- Bot identifier for multi-bot support
    title VARCHAR(500),  -- Conversation title
    conversation_type VARCHAR(50) DEFAULT 'chat',  -- chat, support, etc.
    model_type VARCHAR(50) DEFAULT 'bee',
    started_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    last_activity TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    last_message_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,  -- For ordering
...
DO $$
BEGIN
    -- Add bot_id if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'conversations' AND column_name = 'bot_id') THEN
        ALTER TABLE conversations ADD COLUMN bot_id VARCHAR(255) DEFAULT 'bee';
    END IF;
    ...
    -- Add conversation_type if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'conversations' AND column_name = 'conversation_type') THEN
        ALTER TABLE conversations ADD COLUMN conversation_type VARCHAR(50) DEFAULT 'chat';
    END IF;
    
    -- Add last_message_at if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'conversations' AND column_name = 'last_message_at') THEN
        ALTER TABLE conversations ADD COLUMN last_message_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP;
        -- Backfill from last_activity
        UPDATE conversations SET last_message_at = COALESCE(last_activity, created_at) WHERE last_message_at IS NULL;
    END IF;
END $$;
