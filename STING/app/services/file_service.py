...
class FileService:
        ...
        try:
            ...
            with get_db_session() as session:
                ...
                if not detected_mime_type:
                    ...
                    detected_mime_type = mime_map.get(ext, 'application/octet-stream')
    ...
    def download_file(self, file_id: str, user_id: str, internal_service_bypass: bool = False) -> Optional[Dict[str, Any]]:
        """
        ...
        Args:
            ...
            internal_service_bypass: If True, bypass permission checks for internal services (e.g., QA worker)

        Returns:
            Dict with file data and metadata, or None if not accessible
        """
        try:
            logger.info(f"[FILE_DOWNLOAD] Starting download: file_id={file_id}, user_id={user_id}, internal_bypass={internal_service_bypass}")

            with get_db_session() as session:
                # Get file metadata first (needed for bypass check)
                file_asset = get_file_by_id(session, file_id)
                if not file_asset:
                    logger.error(f"[FILE_DOWNLOAD] File {file_id} not found in database")
                    return None

                # Internal service bypass - for QA worker, admin services, etc.
                if internal_service_bypass:
                    logger.info(f"[FILE_DOWNLOAD] Internal service bypass granted for file {file_id}")
                else:
                ...
                # Get file metadata with detailed logging (already fetched for bypass check above)
                if not file_asset:
                    logger.error(f"[FILE_DOWNLOAD] File {file_id} not found in database")
                    return None
    ...
    def list_user_files(self, user_id: str, file_type: str = None,
        ...
        """
        try:
            logger.info(f"[LIST_FILES] Querying files for user_id={user_id}, file_type={file_type}")
            with get_db_session() as session:
                files = get_user_files(session, user_id, file_type, limit, offset)
                logger.info(f"[LIST_FILES] Query returned {len(files)} files")
                result = [file_asset.to_dict(include_metadata=False) for file_asset in files]
                return result
                
        except Exception as e:
            logger.error(f"[LIST_FILES] Error listing files for user {user_id}: {e}", exc_info=True)
            return []
...
class ProfileFileService:
    ...
    def get_profile_picture(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        logger.info(f"[PROFILE_PIC_SERVICE] Looking for profile picture for user: {user_id}")
        files = self.file_service.list_user_files(user_id, 'profile_picture', limit=1)
        logger.info(f"[PROFILE_PIC_SERVICE] list_user_files returned: {len(files) if files else 0} files")
        if not files:
            logger.warning(f"[PROFILE_PIC_SERVICE] No profile pictures found in database for user {user_id}")
            return None
        
        logger.info(f"[PROFILE_PIC_SERVICE] Found file: {files[0]}")
        # Use file_id (UUID) not id (integer) for download
        file_id = files[0].get('file_id') or files[0].get('id')
        return self.file_service.download_file(file_id, user_id)
