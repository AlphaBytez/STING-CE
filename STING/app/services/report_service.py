...
# ReviewBee configuration (enterprise feature)
REVIEW_BEE_ENABLED = os.environ.get('REVIEW_BEE_ENABLED', 'false').lower() == 'true'
...
class ReportJobData:
    ...
    priority: str = 'normal'
    review_bee_enabled: bool = False  # Enable ReviewBee post-processing
    pii_context: Optional[Dict[str, Any]] = None  # PII context for ReviewBee
...
class ReportService:
    ...
    def update_progress(self, report_id: str, progress_percentage: int, status_message: str = None, current_stage: str = None) -> bool:
        """Update job progress with optional stage and message
        
        Stages: queued, collecting, generating, reviewing, formatting, saving, completed
        """
        try:
            with get_db_session() as session:
                report = get_report_by_id(session, report_id)
    ...
    async def run_review_bee(
        ...
        """
        Run ReviewBee Critic-Revise cycle on report content (enterprise feature).
        
        ReviewBee uses a two-stage architecture:
        1. Lightweight critic model (phi4) reviews and identifies issues
        2. Feedback is appended to original prompt
        3. Powerful LLM regenerates improved content
        
        This approach leverages each model's strengths:
        - Small model: Fast analysis/critique (what it's good at)
        - Large model: High-quality generation (what it's good at)
        ...
        Args:
            ...
            user_id: ID of the report owner
            generator_model: Model used for revision (defaults to report model)
            
        Returns:
            Dict with reviewed content and critique/revision details
        """
        if not REVIEW_BEE_ENABLED:
            logger.debug("ReviewBee is disabled - skipping review")
            return {
                'success': True,
                'content': content,
                'review_performed': False,
                'reason': 'ReviewBee is disabled'
            }
        
        try:
            from app.services.review_bee_service import get_review_bee_service
            
            review_bee = get_review_bee_service()
            
            if not review_bee.is_enabled():
                return {
                    'success': True,
                    'content': content,
                    'review_performed': False,
                    'reason': 'ReviewBee service is not enabled'
                }
            
            logger.info(f"🐝 Running ReviewBee Critic-Revise on report {report_id[:8]}...")
            
            # Run the critic-revise cycle
            result = await review_bee.review_and_revise(
                report_id=report_id,
                content=content,
            ...
            logger.info(f"🐝 ReviewBee complete: score={result.critique.overall_score:.0%}, "
                       f"revised={result.revision_performed}, findings={len(result.critique.findings)}")
            ...
            logger.debug("ReviewBee service not available - skipping review")
            return {
                'success': True,
                'content': content,
                'review_performed': False,
                'reason': 'ReviewBee service not available'
            }
        except Exception as e:
            logger.error(f"ReviewBee review failed: {e}")
            # Don't fail the report - return original content
            return {
                'success': False,
                'content': content,
                'review_performed': False,
