...
class ReviewService:
    ...
    def queue_review(
        ...
        Args:
            ...
            original_request: The original user request/message for context-aware review
    ...
    def get_next_review(self, worker_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                ...
                'original_request': review.original_request,
    ...
    def _get_report_content(self, report_id: str) -> Optional[Dict]:
        ...
        # First, try to use result_summary (most reliable for review)
        if report.result_summary:
            logger.info(f"Using result_summary for report {report_id}")
            content = json.dumps(report.result_summary)

        # If no summary, try to retrieve file from Vault/storage
        if not content and report.result_file_id:
            try:
                # Get file metadata from database
                file_asset = FileAsset.query.filter_by(file_id=report.result_file_id).first()
                if file_asset:
                    # Try to retrieve file using file_service (handles Vault)
                    try:
                        file_service = get_file_service()
                        # download_file returns dict with content, metadata, etc.
                        # Use internal_service_bypass=True for QA worker access
                        file_data = file_service.download_file(
                            report.result_file_id,
                            str(report.user_id),
                            internal_service_bypass=True
                        )
                        if file_data and file_data.get('content'):
                            content = file_data['content']
                            logger.info(f"Retrieved file content from storage for report {report_id}")
                    except Exception as fs_error:
                        logger.warning(f"Could not retrieve file from storage: {fs_error}")

                    # Fall back to summary if file retrieval failed
                    if not content and report.result_summary:
                        content = json.dumps(report.result_summary)
                else:
                    logger.warning(f"FileAsset not found for report {report_id}")
            except Exception as e:
                logger.error(f"Error retrieving report file: {e}")

        # If still no content, return metadata-only for basic checks
        if not content:
            logger.warning(f"No content available for report {report_id}, doing metadata-only review")
