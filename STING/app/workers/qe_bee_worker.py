...
class QEBeeWorker:
    ...
    async def _process_review(self, job: dict) -> dict:
        ...
        # Request alignment check - Verify response addresses the original request
        original_request = job.get('original_request', '')
        if original_request:
            alignment_result = self._check_request_alignment(original_request, content, target_type, metadata)
            findings.append(alignment_result)
            if not alignment_result['passed']:
                issues.append(alignment_result)
    ...
    def _check_request_alignment(self, original_request: str, content: str, target_type: str, metadata: dict) -> dict:
        """
        Check if the response addresses the original user request.

        This is a lightweight heuristic check - for deeper analysis, use _llm_request_alignment_check.
        """
        if not original_request or not content:
            return {
                'check': 'request_alignment',
                'passed': True,
                'code': 'ALIGNMENT_CHECK_SKIPPED',
                'message': 'No original request available for alignment check',
                'confidence': 50
            }
        ...
        # Check for entity types mentioned in request
        if 'hospital' in request_lower or 'medical' in request_lower or 'clinic' in request_lower:
            key_entities.append(('healthcare_org', ['hospital', 'medical', 'health', 'physician', 'doctor']))
        if 'university' in request_lower or 'college' in request_lower or 'professor' in request_lower or 'faculty' in request_lower:
            key_entities.append(('education_org', ['university', 'college', 'faculty', 'professor', 'academic']))
        if 'company' in request_lower or 'ceo' in request_lower or 'executive' in request_lower or 'leadership' in request_lower:
            key_entities.append(('business_org', ['company', 'ceo', 'executive', 'corporation', 'business']))
        if 'bank' in request_lower or 'financial' in request_lower:
            key_entities.append(('financial_org', ['bank', 'financial', 'banking', 'investment']))

        # Check for specific topics
        topics = []
        if 'doctor' in request_lower or 'physician' in request_lower or 'medical staff' in request_lower:
            topics.append('medical_staff')
        if 'professor' in request_lower or 'faculty' in request_lower:
            topics.append('academic_staff')
        if 'executive' in request_lower or 'leadership' in request_lower or 'ceo' in request_lower:
            topics.append('leadership')
        if 'cardiac' in request_lower or 'heart' in request_lower or 'cardiology' in request_lower:
            topics.append('cardiac')

        # Check for entity name from request (e.g., "Northside Hospital", "Stanford University")
        import re
        entity_patterns = [
            r'(?:about|at|for|the)\s+([A-Z][a-zA-Z\s]+?)(?:\s+(?:hospital|university|company|corporation|bank|institute))?',
        ]
        # Extract potential entity names
        words = request_lower.split()
        potential_entities = []
        for i, word in enumerate(words):
            if word in ['at', 'for', 'about', 'the']:
                if i + 1 < len(words):
                    # Capture multi-word entity names
                    entity_words = []
                    for j in range(i + 1, min(i + 5, len(words))):
                        if words[j] in ['hospital', 'university', 'company', 'corporation', 'bank', 'institute', 'clinic', 'center']:
                            entity_words.append(words[j])
                            break
                        if words[j] in ['and', 'or', 'with', 'for', 'the', 'a', 'an']:
                            break
                        entity_words.append(words[j])
                    if entity_words:
        ...
        # Check if key entity type is addressed
        for entity_type, keywords in key_entities:
            if not any(kw in content_lower for kw in keywords):
                issues_found.append(f"Missing {entity_type.replace('_', ' ')} context")
                alignment_score -= 25

        # Check if requested topics are addressed
        for topic in topics:
            topic_keywords = {
                'medical_staff': ['doctor', 'physician', 'medical staff', 'healthcare'],
                'academic_staff': ['professor', 'faculty', 'academic'],
                'leadership': ['executive', 'leadership', 'ceo', 'director'],
                'cardiac': ['cardiac', 'heart', 'cardiology', 'cardiologist']
            }
            if not any(kw in content_lower for kw in topic_keywords.get(topic, [])):
                issues_found.append(f"Missing {topic.replace('_', ' ')} content")
                alignment_score -= 15

        # Check for hallucinated content patterns
        # Flag if response has generic placeholder names
        placeholder_names = ['dr. john doe', 'dr. jane smith', 'dr. emily johnson', 'generic name', 'example name']
        if any(name in content_lower for name in placeholder_names):
            issues_found.append("Possible fabricated/placeholder content detected")
            alignment_score -= 30

        passed = alignment_score >= 70
        return {
            'check': 'request_alignment',
            'passed': passed,
            'code': 'ALIGNMENT_OK' if passed else 'ALIGNMENT_ISSUE',
            'message': f"Request alignment check: {'Passed' if passed else 'Issues found'}. " + '; '.join(issues_found) if issues_found else 'Response addresses request',
            'confidence': min(alignment_score, 95),
            'details': {
                'alignment_score': alignment_score,
                'issues': issues_found,
                'topics_checked': topics,
                'entities_found': len(key_entities)
            }
        }

    async def _llm_request_alignment_check(self, original_request: str, content: str) -> dict:
        """
        Use LLM to deeply evaluate if response addresses the original request.
        This is more thorough but slower than _check_request_alignment.
        """
        try:
            review_content = content[:1500] if len(content) > 1500 else content
            review_request = original_request[:500] if len(original_request) > 500 else original_request

            prompt = f"""You are a quality assurance reviewer. Evaluate if the response addresses the original request.
...
2. Are the key entities (organizations, people, topics) from the request mentioned?
3. Is there any fabricated or hallucinated content?

Respond with ONLY a JSON object (no markdown):
{{"passed": true/false, "score": 1-10, "reason": "brief explanation", "concerns": ["list of potential issues"]}}"""
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    return {
                        'check': 'request_alignment_llm',
                        'passed': passed and score >= 5,
                        'code': 'ALIGNMENT_LLM_OK' if passed else 'ALIGNMENT_LLM_FAILED',
                        'message': result.get('reason', 'LLM alignment check completed'),
            ...
            return {
                'check': 'request_alignment_llm',
                'passed': True,
                'code': 'ALIGNMENT_LLM_SKIPPED',
                'message': 'LLM alignment check unavailable',
                'confidence': 50
            }
            ...
            return {
                'check': 'request_alignment_llm',
                'passed': True,
                'code': 'ALIGNMENT_LLM_ERROR',
                'message': f'LLM alignment check error: {str(e)[:50]}',
                'confidence': 50
            }
