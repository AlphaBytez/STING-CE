...
class BaseReportGenerator(ABC):
    ...
    def update_progress(self, percentage: int, message: str, stage: str = "generating"):
        """Update progress if callback is available"""
        if self.progress_callback:
            try:
                self.progress_callback(percentage, message, stage)
    ...
    async def _generate_ai_insights(self, summary_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            response = requests.post(
                ...
                json={
                    'message': prompt,
                    'user_id': 'system_report_generator',  # System user to avoid polluting chat
                    'context': {
                        'generation_mode': 'insight',
                        'bypass_token_limit': False,
                        'skip_conversation_save': True,
                        'skip_web_search': True  # Don't trigger web search for internal calls
                    },
                    'require_auth': False
                },
                timeout=30
...
class BeeConversationalReportGenerator(BaseReportGenerator):
    ...
    async def collect_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Start background progress updater since the HTTP call is blocking
            # Progress will increment from 15% to 55% over the expected generation time
            stop_progress_event = threading.Event()
            progress_thread = threading.Thread(
                target=self._update_progress_during_generation,
    ...
    def _update_progress_during_generation(self, stop_event: threading.Event):
        """Background thread to update progress during long AI generation.
        
        Simulates progress from 15% to 55% over time since we can't get
        real streaming progress from the HTTP API call.
        
        Progress stages:
        - 15-25%: Web search and context gathering (fast, ~10s)
        - 25-40%: LLM processing prompt (medium, ~30s)
        - 40-55%: Content generation (slow, varies by length)
        """
        import time
        
        progress_stages = [
            (15, "Searching web for relevant sources...", 5),
            (20, "Gathering context from knowledge base...", 5),
            (25, "Preparing AI prompt with context...", 5),
            (30, "AI is analyzing the request...", 10),
            (35, "Generating initial content structure...", 10),
            (40, "Writing executive summary...", 15),
            (45, "Developing detailed analysis...", 20),
            (50, "Adding recommendations and insights...", 20),
            (55, "Finalizing content generation...", 30),
        ]
        
        for progress, message, delay in progress_stages:
            if stop_event.is_set():
                return
            
            self.update_progress(progress, message, "generating")
        ...
        # If we're still going, slowly increment from 55% to 58%
        # (report might be very long)
        current_progress = 55
        while not stop_event.is_set() and current_progress < 58:
            current_progress += 1
            self.update_progress(current_progress, "Still generating content (large report)...", "generating")
...
Respond with ONLY the title, nothing else. Make it descriptive and professional. The title should directly reflect the ACTUAL topic of the report content above.

Examples of good title formats:
- "[Topic] Security Analysis"
- "[Technology] Implementation Best Practices"  
- "[Industry/Domain] Framework Overview"
...
Your title:"""
            ...
            response = requests.post(
                ...
                json={
                    ...
                    'user_id': 'system_title_generator',  # Use system user to avoid polluting user's chat
                    'context': {
                        'generation_mode': 'title',
                        'bypass_token_limit': False,
                        'skip_conversation_save': True,  # Don't save to conversation history
                        'skip_web_search': True  # Don't trigger web search for internal calls
                    },
                    'require_auth': False
                },
