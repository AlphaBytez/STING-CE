...
class ReportWorker:
    ...
    async def process_job(self, job: Dict[str, Any]):
        ...
        try:
            ...
            # Stage 1: Starting (5-10%)
            self.report_service.update_progress(report_id, 5, "Initializing report generation", "collecting")
            ...
            # Create a progress callback for the generator to use
            def progress_callback(percentage: int, message: str, stage: str):
                self.report_service.update_progress(report_id, percentage, message, stage)

            # Initialize generator with parameters and progress callback
            ...
            # Stage 2: Generating (10-60%) - AI content generation is the longest phase
            if template_name == 'bee_conversational_report':
                self.report_service.update_progress(report_id, 10, "Starting AI content generation...", "generating")
            else:
                self.report_service.update_progress(report_id, 10, "Collecting data", "collecting")
            ...
            # Stage 3: ReviewBee quality check (60-75%) - if enabled
            review_bee_enabled = os.getenv('REVIEW_BEE_ENABLED', 'true').lower() == 'true'
            if review_bee_enabled and template_name == 'bee_conversational_report':
                self.report_service.update_progress(report_id, 60, "ReviewBee quality check in progress...", "reviewing")
            else:
                self.report_service.update_progress(report_id, 65, "Content generated", "formatting")

            # Stage 4: Formatting (75-85%)
            self.report_service.update_progress(report_id, 75, "Formatting report document", "formatting")
            ...
            self.report_service.update_progress(report_id, 85, "Saving report to storage", "saving")
    ...
    def _render_inline_markdown(self, tokens):
        ...
        result = []
        current_link_href = None  # Track link URL
        ...
        for token in tokens:
            if token.type == 'text':
                ...
                if hasattr(token, 'attrs') and token.attrs:
                    # attrs can be dict or list of tuples depending on markdown_it version
                ...
                if href:
                    ...
                    # ReportLab supports <a href="..."> tags in Paragraphs
                    result.append(f'<a href="{href}" color="blue">')
    ...
    async def create_output_file(self, report_data: Dict[str, Any],
        ...
        if output_format == 'csv':
            ...
            # Clean metadata - just the date, right-aligned
            metadata_style = ParagraphStyle(
                'STINGMetadata',
                parent=styles['Normal'],
                fontSize=10,
                textColor=colors.HexColor('#6b7280'),  # Subtle gray
                spaceAfter=0.3*inch,
                leading=14,
