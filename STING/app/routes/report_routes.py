...
@report_bp.route('/public/config', methods=['GET'])
def get_public_report_config():
    """Get report system configuration for UI indicators - no auth required"""
    try:
        # Get ReviewBee status from environment
        review_bee_enabled = os.environ.get('REVIEW_BEE_ENABLED', 'false').lower() == 'true'
        review_bee_mode = os.environ.get('REVIEW_BEE_MODE', 'critique_and_revise')
        review_bee_threshold = float(os.environ.get('REVIEW_BEE_THRESHOLD', '0.75'))
        
        return jsonify({
            'success': True,
            'data': {
                'review_bee': {
                    'enabled': review_bee_enabled,
                    'mode': review_bee_mode,
                    'threshold': review_bee_threshold,
                    'description': 'AI-powered quality assurance with hallucination detection' if review_bee_enabled else None
                },
                'features': {
                    'pii_protection': True,
                    'pdf_generation': True,
...
def get_current_user() -> Optional[str]:
    """Get current user ID - ALWAYS returns kratos_id as canonical identifier.
    
    This ensures consistency across all auth methods:
    - API key auth: looks up user's kratos_id
    - Session auth: uses g.user.kratos_id
    - Kratos cookie: uses identity.id (which IS the kratos_id)
    """
    
    # Handle API key authentication
    if hasattr(g, 'api_key') and g.api_key:
        # API key stores user_id which should be kratos_id
        logger.info(f"[AUTH] API key user: {g.api_key.user_id}")
        return str(g.api_key.user_id)
    
    # Handle session authentication - use kratos_id
    if hasattr(g, 'user') and g.user:
        if g.user.kratos_id:
            logger.info(f"[AUTH] Session user (kratos_id): {g.user.kratos_id}")
            return g.user.kratos_id
        logger.error(f"[AUTH] User {g.user.id} has no kratos_id - this should not happen")
        return None
    
    # Fallback to Kratos session cookie
...
@require_auth_method(['webauthn', 'totp'])  # Tier 2 protection - requires second factor
def list_reports():
    """List reports for the current user - optimized for performance"""
    try:
        user_id = get_current_user()
        if not user_id:
            return jsonify({'error': 'Authentication required'}), 401
        ...
        with get_db_session() as session:
            ...
            # Template is eager-loaded via lazy='joined' in model relationship
            reports = query.order_by(Report.created_at.desc())\
                          .offset(offset)\
                          .limit(limit)\
                          .all()
            
            # Use include_template=False for list view to reduce payload size
            # Template name/display_name are still included for display
            return jsonify({
                'success': True,
                'data': {
                    'reports': [report.to_dict(include_template=False) for report in reports],
                    'pagination': {
                        'total': total_count,
                        'limit': limit,
                        'offset': offset,
