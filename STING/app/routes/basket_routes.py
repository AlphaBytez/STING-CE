...
def get_basket_overview():
    ...
    """
    try:
        # g.user.id is the kratos_id (UUID), not the database user ID
        kratos_id = g.user.id if hasattr(g, 'user') and g.user else None
        
        if not kratos_id:
            return jsonify({'error': 'User not authenticated'}), 401

        current_app.logger.info(f"Basket overview request from kratos_id {kratos_id}")

        # Look up the actual database user by kratos_id
        user = User.query.filter_by(kratos_id=str(kratos_id)).first()
        if not user:
            current_app.logger.warning(f"No database user found for kratos_id {kratos_id}")
            return jsonify({'error': 'User not found in database'}), 404
        
        user_id = user.id  # This is the integer database ID
        user_quota = 1073741824  # Default 1GB
        # Check for user-specific storage quota in user preferences/settings
        # Note: user.metadata may not be a dict, so handle carefully
        try:
            if hasattr(user, 'preferences') and user.preferences and isinstance(user.preferences, dict):
                user_quota = user.preferences.get('storage_quota', user_quota)
        except Exception:
            pass  # Use default quota

        # Get user's documents across their own honey jars only
        # Do NOT include system jars - those are demo data and should not clutter user's basket
        user_email = user.email if user else None
        user_kratos_id = user.kratos_id if user else None
        
        # Build owner conditions - only match user's own jars
        owner_conditions = [
            HoneyJar.owner == str(user_id),
            HoneyJar.owner == user_email,
        ]
        if user_kratos_id:
            owner_conditions.append(HoneyJar.owner == str(user_kratos_id))
...
@basket_bp.route('/documents/<document_id>/download', methods=['GET'])
@require_auth
def download_document(document_id):
    """
    Download a document from the user's basket
    """
    try:
        user_id = g.user.id if hasattr(g, 'user') and g.user else None
        ...
        document = db.session.query(Document).join(
            HoneyJar, Document.honey_jar_id == HoneyJar.id
        ).filter(
            and_(
        ...
        content = None
        if document.doc_metadata and 'content' in document.doc_metadata:
            content = document.doc_metadata['content']
        elif document.doc_metadata and 'source' in document.doc_metadata:
            # Report from Bee chat - reconstruct from metadata
            content = document.doc_metadata.get('content', f'# {document.filename}\n\n[Content stored in metadata]')

        # If no content stored, return a placeholder
        if not content:
            content = f"# {document.filename}\n\n[File content not available - document entry only]"
...
@basket_bp.route('/documents/<document_id>/view', methods=['GET'])
@require_auth
def view_document(document_id):
    """
    View a document from the basket (returns JSON with content for inline display)
    """
    try:
        user_id = g.user.id if hasattr(g, 'user') and g.user else None
        ...
        return jsonify({
            ...
            'size_formatted': format_bytes(document.size_bytes or 0),
            'content': content,
            'content_type': document.content_type,
            'honey_jar_name': document.honey_jar.name if document.honey_jar else 'Unknown',
            'created_at': document.upload_date.isoformat() if document.upload_date else None,
            'can_download': True
        })

    except Exception as e:
        current_app.logger.error(f"View document error: {str(e)}")
        return jsonify({
...
@basket_bp.route('/documents/<document_id>', methods=['DELETE'])
@require_auth
def delete_document(document_id):
    """
    Delete a document from the basket (soft delete)
    """
    try:
        user_id = g.user.id if hasattr(g, 'user') and g.user else None
        ...
        document = db.session.query(Document).join(
            ...
            and_(
                Document.id == document_id,
                HoneyJar.owner == str(user_id)  # Must own the jar to delete
            )
        ).first()
