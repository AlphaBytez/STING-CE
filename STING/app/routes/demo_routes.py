...
from app.services.demo_data_generator import MedicalDemoGenerator, LegalDemoGenerator, FinancialDemoGenerator, HRBenefitsGenerator, ITSecurityGenerator, SalesGenerator
...
def generate_demo_data():
    ...
    try:
        ...
        if scenario == 'basic':
            ...
            # Legal-financial demo with dedicated legal and financial focus
            result['generated'] = _generate_legal_financial_demo_step(step, current_user_id)
        else:
...
def _generate_legal_financial_demo_step(step, user_id=None):
    """Generate Legal & Financial demo data step by step - 20 honey jars, 120 documents"""
    generated = {'honeyJars': 0, 'documents': 0, 'reports': 0, 'users': 0, 'nectarBots': 0}

    try:
        if step == 1:  # Create legal and financial honey jars (10 + 10 = 20)
            jars_created = _create_legal_financial_honey_jars()
            generated['honeyJars'] = jars_created

        elif step == 2:  # Generate legal and financial documents (120)
            docs_created = _create_legal_financial_documents()
            generated['documents'] = docs_created

        elif step == 3:  # Create compliance reports
            reports_created = _create_sample_reports('legal-financial', user_id)
            bots_created = _create_demo_nectar_bots('legal-financial')
            generated['reports'] = reports_created
            generated['nectarBots'] = bots_created

        elif step == 4:  # Create compliance and regulatory samples
            _create_legal_financial_compliance_data()

        elif step == 5:  # Finalize
            _finalize_demo_setup('legal-financial')

    except Exception as e:
        current_app.logger.error(f"Error in legal-financial demo step {step}: {str(e)}")
...
def _find_or_create_honey_jar(honey_jar_service, name: str, description: str, jar_type: str = "public", tags: list = None) -> tuple:
    """
    Find existing honey jar by name or create new one.
    Returns (jar_id, was_created) tuple.
    """
    import requests
    ...
    try:
        ...
        response = requests.get(
            f"{honey_jar_service.knowledge_url}/honey-jars?limit=500",
            headers=headers,
            timeout=10
        )
        ...
        if response.status_code == 200:
            jars = response.json()
            for jar in jars:
                if jar.get('name') == name:
                    current_app.logger.info(f"Found existing honey jar: {name} (ID: {jar.get('id')})")
                    return jar.get('id'), False
        ...
        current_app.logger.error(f"Error finding/creating honey jar {name}: {e}")
        return None, False
...
def _create_basic_honey_jars():
    ...
    for jar_config in basic_jars:
        try:
            jar_id, was_created = _find_or_create_honey_jar(
                honey_jar_service,
...
def _create_legal_financial_honey_jars():
    """Create 20 honey jars for legal-financial demo (10 legal + 10 financial)"""
    legal_jars = [
        {'name': 'Legal - Contracts Archive', 'description': 'Commercial contracts and agreements', 'tags': ['legal', 'contracts', 'demo']},
        {'name': 'Legal - Litigation Files', 'description': 'Active and closed litigation documents', 'tags': ['legal', 'litigation', 'demo']},
        {'name': 'Legal - Corporate Governance', 'description': 'Board minutes and corporate filings', 'tags': ['legal', 'governance', 'demo']},
        {'name': 'Legal - Intellectual Property', 'description': 'Patents, trademarks, and IP documentation', 'tags': ['legal', 'ip', 'demo']},
        {'name': 'Legal - Compliance Records', 'description': 'Regulatory compliance documentation', 'tags': ['legal', 'compliance', 'demo']},
        {'name': 'Legal - NDAs & Confidential', 'description': 'Non-disclosure agreements and confidential materials', 'tags': ['legal', 'nda', 'confidential', 'demo']},
        {'name': 'Legal - Employment Agreements', 'description': 'Employment contracts and HR legal docs', 'tags': ['legal', 'employment', 'demo']},
        {'name': 'Legal - Real Estate', 'description': 'Property leases and real estate documents', 'tags': ['legal', 'real-estate', 'demo']},
        {'name': 'Legal - Regulatory Filings', 'description': 'SEC and regulatory filing documents', 'tags': ['legal', 'regulatory', 'demo']},
        {'name': 'Legal - M&A Documents', 'description': 'Merger and acquisition documentation', 'tags': ['legal', 'ma', 'demo']}
    ]
    
    financial_jars = [
        {'name': 'Finance - Financial Statements', 'description': 'Balance sheets, income statements, cash flow', 'tags': ['financial', 'statements', 'demo']},
        {'name': 'Finance - Audit Reports', 'description': 'Internal and external audit documentation', 'tags': ['financial', 'audit', 'demo']},
        {'name': 'Finance - Tax Records', 'description': 'Tax filings and supporting documentation', 'tags': ['financial', 'tax', 'demo']},
        {'name': 'Finance - Budget Planning', 'description': 'Annual budgets and forecasts', 'tags': ['financial', 'budget', 'demo']},
        {'name': 'Finance - Accounts Payable', 'description': 'Vendor invoices and payment records', 'tags': ['financial', 'ap', 'demo']},
        {'name': 'Finance - Accounts Receivable', 'description': 'Customer invoices and collections', 'tags': ['financial', 'ar', 'demo']},
        {'name': 'Finance - Investment Records', 'description': 'Investment portfolios and returns', 'tags': ['financial', 'investments', 'demo']},
        {'name': 'Finance - Loan Documents', 'description': 'Loan applications and credit agreements', 'tags': ['financial', 'loans', 'pci', 'demo']},
        {'name': 'Finance - Treasury Reports', 'description': 'Cash management and treasury operations', 'tags': ['financial', 'treasury', 'demo']},
        {'name': 'Finance - Compliance Reports', 'description': 'SOX compliance and financial controls', 'tags': ['financial', 'compliance', 'sox', 'demo']}
    ]

    created = 0
    honey_jar_service = get_honey_jar_service()

    for jar_config in legal_jars + financial_jars:
        try:
            jar_id, was_created = _find_or_create_honey_jar(
                honey_jar_service,
                name=jar_config['name'],
            ...
            current_app.logger.error(f"Error creating legal-financial honey jar {jar_config['name']}: {str(e)}")
...
def _create_sample_documents(scenario_type):
    ...
    try:
        ...
        hr_gen = HRBenefitsGenerator()
        it_sec_gen = ITSecurityGenerator()
        sales_gen = SalesGenerator()
        
        # Create rotating generators for variety within each category
        hr_generators = [
            hr_gen.generate_benefits_overview,
            hr_gen.generate_pto_policy,
            hr_gen.generate_employee_handbook_excerpt,
            hr_gen.generate_compensation_policy,
            hr_gen.generate_onboarding_checklist
        ]
        
        it_sec_generators = [
            it_sec_gen.generate_security_incident_report,
            it_sec_gen.generate_security_policy,
            it_sec_gen.generate_access_review_report,
            it_sec_gen.generate_vulnerability_report
        ]
        
        sales_generators = [
            sales_gen.generate_proposal,
            sales_gen.generate_quote
        ]
        
        def make_rotating_generator(generators):
            """Create a rotating generator function"""
            counter = [0]  # Use list to allow modification in closure
            def gen():
                result = generators[counter[0] % len(generators)]()
                counter[0] += 1
                return result
            return gen
        ...
        if scenario_type == 'comprehensive':
            ...
            targets = [
                ('HR', 'Benefits', make_rotating_generator(hr_generators), 45, ['hr', 'benefits', 'demo']),
                ...
                ('IT', 'Security', make_rotating_generator(it_sec_generators), 45, ['technical', 'security', 'demo']),
                ('Sales', 'Proposals', make_rotating_generator(sales_generators), 45, ['sales', 'commercial', 'demo'])
            ]
        else:
...
def _create_legal_financial_documents():
    """Create 120 legal and financial documents (60 legal + 60 financial)"""
    documents_created = 0
    ...
    try:
        ...
        # Legal document generators - rotate through different types
        legal_generators = [
            ('contract', legal_gen.generate_contract),
            ('case_file', legal_gen.generate_case_file),
            ('client_memo', legal_gen.generate_client_memo),
            ('nda', legal_gen.generate_nda),
            ('settlement_agreement', legal_gen.generate_settlement_agreement),
        ]
        
        # Financial document generators - rotate through different types
        financial_generators = [
            ('loan_application', financial_gen.generate_loan_application),
            ('credit_report', financial_gen.generate_credit_report),
            ('financial_statement', financial_gen.generate_financial_statement),
            ('bank_statement', financial_gen.generate_bank_statement),
            ('tax_document', financial_gen.generate_tax_document),
        ]

        # Legal honey jars to distribute documents to
        legal_jars = [
        ...
        # Financial honey jars to distribute documents to
        financial_jars = [
            ...
            'Finance - Investment Records',
            'Finance - Loan Documents',
            'Finance - Treasury Reports',
            'Finance - Compliance Reports'
        ]

        # Generate 60 legal documents, 6 per jar
        for jar_idx, jar_name in enumerate(legal_jars):
            for doc_idx in range(6):
                try:
                    gen_idx = (jar_idx + doc_idx) % len(legal_generators)
                    doc_type, generator_func = legal_generators[gen_idx]
                    doc_content = generator_func()
                    filename = f"legal_{doc_type}_{jar_idx+1}_{doc_idx+1}_{uuid.uuid4().hex[:6]}.txt"
                    
                    if _save_demo_document(jar_name, filename, doc_content, ['legal', 'privileged', 'demo']):
                        documents_created += 1
                except Exception as e:
                    current_app.logger.error(f"Error creating legal document for {jar_name}: {e}")

        # Generate 60 financial documents, 6 per jar
        for jar_idx, jar_name in enumerate(financial_jars):
            for doc_idx in range(6):
                try:
                    gen_idx = (jar_idx + doc_idx) % len(financial_generators)
                    doc_type, generator_func = financial_generators[gen_idx]
                    doc_content = generator_func()
                    filename = f"financial_{doc_type}_{jar_idx+1}_{doc_idx+1}_{uuid.uuid4().hex[:6]}.txt"
                    
                    if _save_demo_document(jar_name, filename, doc_content, ['financial', 'pci', 'demo']):
                        documents_created += 1
                except Exception as e:
                    current_app.logger.error(f"Error creating financial document for {jar_name}: {e}")

        current_app.logger.info(f"Created {documents_created} legal-financial demo documents")

    except Exception as e:
        current_app.logger.error(f"Error creating legal-financial documents: {str(e)}")

    return documents_created

def _create_legal_financial_compliance_data():
    """Create compliance and regulatory data samples for legal-financial demo"""
    try:
        # Log compliance data creation
        current_app.logger.info("Creating legal-financial compliance data samples")
        
        # This could be expanded to create audit trail entries, 
        # compliance check records, etc.
        
    except Exception as e:
        current_app.logger.error(f"Error creating legal-financial compliance data: {str(e)}")
...
@demo_bp.route('/cleanup-empty-jars', methods=['POST'])
@require_admin
def cleanup_empty_honey_jars():
    """
    Clean up duplicate and empty honey jars.
    Keeps one instance of each jar name and removes all empty duplicates.
    """
    try:
        import requests
        ...
        if response.status_code != 200:
            return jsonify({'error': 'Could not fetch honey jars', 'status': response.status_code}), 500
        ...
        for jar in jars:
            name = jar.get('name', '')
            if name not in jars_by_name:
                jars_by_name[name] = []
            jars_by_name[name].append(jar)
        
        deleted_count = 0
        kept_count = 0
        
        for name, jar_list in jars_by_name.items():
            # Sort by document_count descending (keep the one with most documents)
            jar_list.sort(key=lambda x: x.get('document_count', 0), reverse=True)
            
            # Keep the first one (most documents), delete the rest
            kept_count += 1
            
            for jar in jar_list[1:]:  # Skip first, delete rest
                jar_id = jar.get('id')
                doc_count = jar.get('document_count', 0)
                ...
                if jar_id:
                    ...
                    if delete_response.status_code in [200, 204]:
                        deleted_count += 1
                        current_app.logger.info(f"Deleted duplicate jar: {name} (ID: {jar_id[:8]}..., docs: {doc_count})")
                    else:
                        current_app.logger.warning(f"Failed to delete jar {name}: {delete_response.status_code}")
        
        current_app.logger.info(f"Cleanup complete: kept {kept_count} jars, deleted {deleted_count} duplicates")
        
        return jsonify({
            'success': True,
            'message': f'Cleanup complete: kept {kept_count} jars, deleted {deleted_count} duplicates',
            'kept': kept_count,
            'deleted': deleted_count
        })
        
    except Exception as e:
        current_app.logger.error(f"Error cleaning up empty jars: {str(e)}")
