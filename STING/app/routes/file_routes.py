...
def get_current_user() -> Optional[str]:
    """Get current user ID from Flask or Kratos session.
    
    ALWAYS returns kratos_id as the canonical user identifier.
    This ensures consistency across all auth methods and file storage.
    """
    from flask import g, session
    
    # PRIORITY 1: Check Flask g.user (set by auth middleware)
    if hasattr(g, 'user') and g.user:
        if g.user.kratos_id:
            return g.user.kratos_id
        logger.error(f"User {g.user.id} has no kratos_id - this should not happen")
        return None
    
    # PRIORITY 2: Try Flask session directly if g.user not set (WebAuthn auth)
    if session.get('user_id') and session.get('auth_method') == 'enhanced_webauthn':
        try:
            from app.models.user_models import User
            user = User.query.get(session.get('user_id'))
            if user and user.kratos_id:
                return user.kratos_id
            elif user:
                logger.error(f"User {user.id} has no kratos_id - this should not happen")
        except Exception as e:
            logger.error(f"Error loading user from Flask session in get_current_user: {e}")
    
    # PRIORITY 3: Fall back to Kratos session cookie
    ...
    # Kratos identity ID is the canonical kratos_id
...
def get_profile_picture():
    ...
    try:
        user_id = get_current_user()
        logger.info(f"[PROFILE_PIC] GET request, user_id from auth: {user_id}")
        if not user_id:
            logger.warning("[PROFILE_PIC] No user_id from get_current_user()")
            return jsonify({'error': 'Authentication required'}), 401
        
        file_data = get_profile_service().get_profile_picture(user_id)
        if not file_data:
            logger.warning(f"[PROFILE_PIC] No profile picture found for user {user_id}")
            return jsonify({'error': 'No profile picture found'}), 404
        
        # Create file-like object
        file_obj = BytesIO(file_data['data'])
        
        # Determine mime_type with fallback
        ...
        if not mime_type:
            ...
            if not mime_type:
                mime_type = 'image/jpeg'  # Default for profile pictures
        
        return send_file(
            file_obj,
            mimetype=mime_type,
            as_attachment=False,
            download_name=file_data.get('filename', 'profile.jpg')
        )
