...
def chat_with_bee():
    ...
    try:
        ...
        # Check for demo mode - route to demo AI service if enabled
        import os
        demo_config_path = os.environ.get('INSTALL_DIR', '/opt/sting-ce') + '/conf/config.demo.yml'
        is_demo_mode = False
        try:
            import yaml
            if os.path.exists(demo_config_path):
                with open(demo_config_path, 'r') as f:
                    ...
                    is_demo_mode = demo_config.get('demo_mode', {}).get('enabled', False) or \
                                   demo_config.get('application', {}).get('demo_features', {}).get('enabled', False)
        except (ImportError, Exception):
            pass

        # Check for demo session cookie
        demo_session_cookie = request.cookies.get('sting_demo_session')

        # Demo mode: route to demo AI service
        if is_demo_mode and demo_session_cookie:
            logger.info("Demo mode active - routing to demo AI service")
            DEMO_AI_SERVICE_URL = os.getenv('DEMO_AI_SERVICE_URL', 'http://demo-ai:8095')

            try:
                response = requests.post(
                    f"{DEMO_AI_SERVICE_URL}/bee/chat",
                    json={'message': message, 'conversation_id': conversation_id},
                    timeout=10
                )

                if response.status_code == 200:
                    demo_response = response.json()
                    # Add demo mode flag to response
                    demo_response['demo_mode'] = True
                    demo_response['conversation_id'] = conversation_id
                    return jsonify(demo_response)
                else:
                    logger.warning(f"Demo AI service returned {response.status_code}")
            except requests.exceptions.RequestException as e:
                logger.warning(f"Demo AI service unavailable: {e}")
                # Fall through to regular chat (demo AI failed)
