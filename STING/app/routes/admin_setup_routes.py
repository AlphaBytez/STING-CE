...
def _send_email_code(email, code):
    ...
    try:
        # Get SMTP settings from environment (uses SMTP_* vars set by config_loader.py)
...
def _check_admin_exists():
    """Check if admin user already exists in the app database"""
    try:
        # Check the app's users table for any admin or super_admin users
