    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if session_source:
                ...
                # First try session data (may be empty for whoami responses)
                identity = session_source.get('identity', {})
                credentials = identity.get('credentials', {})

                logger.info(f"🔍 TIERED AUTH: Available credentials from session: {list(credentials.keys()) if credentials else 'None'}")

                # If no credentials in session, check Redis cache first, then fetch from Kratos Admin API
                # This is needed because whoami doesn't include credentials
                if not credentials and g.user and hasattr(g.user, 'kratos_id') and g.user.kratos_id:
                    # Check Redis cache for credentials (5 minute TTL)
                    try:
                        import redis
                        import json
                        redis_client = redis.from_url(os.environ.get('REDIS_URL', 'redis://redis:6379/0'))
                        cache_key = f"sting:kratos_credentials:{g.user.kratos_id}"
                        cached_creds = redis_client.get(cache_key)
                        
                        if cached_creds:
                            credentials = json.loads(cached_creds)
                            logger.info(f"🔍 TIERED AUTH: Using cached credentials from Redis: {list(credentials.keys()) if credentials else 'None'}")
                        else:
                            # Cache miss - fetch from Kratos Admin API
                            try:
                                import requests
                                ...
                                response = requests.get(
                                    ...
                                    verify=False,
                                    timeout=3  # Reduced timeout since we have caching
                                )
                                if response.status_code == 200:
                                    ...
                                    redis_client.setex(cache_key, 300, json.dumps(credentials))
                                    logger.info(f"🔍 TIERED AUTH: Fetched credentials from Kratos Admin and cached: {list(credentials.keys()) if credentials else 'None'}")
                                else:
                                    logger.warning(f"🔍 TIERED AUTH: Failed to fetch credentials from Kratos Admin: {response.status_code}")
                            except Exception as e:
                                logger.warning(f"🔍 TIERED AUTH: Error fetching credentials from Kratos Admin: {e}")
                    except Exception as redis_error:
                        logger.warning(f"🔍 TIERED AUTH: Redis cache unavailable, falling back to direct Kratos call: {redis_error}")
                        # Fallback to direct call without caching
                        try:
                            import requests
                            KRATOS_ADMIN_URL = os.environ.get('KRATOS_ADMIN_URL', 'https://kratos:4434')
                            response = requests.get(
                            ...
                            if response.status_code == 200:
                                identity_data = response.json()
                                credentials = identity_data.get('credentials', {})
                                logger.info(f"🔍 TIERED AUTH: Fetched credentials from Kratos Admin (no cache): {list(credentials.keys()) if credentials else 'None'}")
                        except Exception as e:
                            logger.warning(f"🔍 TIERED AUTH: Error fetching credentials from Kratos Admin: {e}")
