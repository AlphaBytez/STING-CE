...
class PIIDetector:
    ...
    # Organization type keywords that indicate this is NOT a person name
    ORGANIZATION_KEYWORDS = [
        'hospital', 'medical', 'health', 'university', 'college', 'school',
        'company', 'corporation', 'inc', 'llc', 'ltd', 'corp',
        'bank', 'trust', 'insurance', 'agency', 'authority',
        'center', 'institute', 'foundation', 'association', 'society',
        'clinic', 'pharmacy', 'laboratory', 'laboratoire',
        'hotel', 'restaurant', 'cafe', 'coffee',
        'airlines', 'airways', 'transportation', 'logistics',
        'restaurant', 'bar', 'grill', 'kitchen',
        'museum', 'gallery', 'theater', 'theatre',
        'hotel', 'resort', 'spa', 'inn',
    ]
    ...
    def _detect_names(self, text: str) -> List[PIIDetection]:
        ...
        Excludes organization names (hospital, university, company, etc.)
        """
        detections = []
        ...
        for match in name_pattern.finditer(text):
            ...
            # Skip if this looks like an organization name
            is_organization = any(
                org_keyword in matched_lower
                for org_keyword in self.ORGANIZATION_KEYWORDS
            )
            if is_organization:
                continue
