...
class ModeDetector:
    ...
    def detect_mode(
        ...
        Args:
            ...
            provider: AI provider name (ollama, openai, anthropic, minimax, etc.)
            ...
            is_cloud_provider: If True, forces 'external' mode for full PII protection

        Returns:
            Tuple of (mode_name, mode_config)
            Example: ("trusted", {...mode_config...})
        """
        # 0. Cloud providers ALWAYS use external mode (highest priority after overrides)
        # This ensures MiniMax, OpenAI, Anthropic, etc. always get full PII protection
        if is_cloud_provider:
            external_mode = self.modes_config.get('external', {
                'enabled': True,
                'protection_level': 'full',
                'serialize_all_pii': True
            })
            logger.info(f"Cloud provider '{provider}' detected - forcing 'external' mode for full PII protection")
            return 'external', external_mode
