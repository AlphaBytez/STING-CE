...
from .pii_analytics import PIIAnalytics, get_analytics_instance

__version__ = "1.0.3"  # Bumped for analytics
__all__ = [
    "PIIMiddleware",
    "PIIDetector",
    "PIISerializer",
...
class PIIMiddleware:
    ...
    async def serialize_message(
        ...
        # Extract PII types for analytics
        pii_types = list(set(d.pii_type.value if hasattr(d.pii_type, 'value') else str(d.pii_type) for d in pii_detections))

        context = {
            "conversation_id": conversation_id,
            "pii_serialized": True,
            "pii_count": len(pii_detections),
            "pii_types": pii_types,  # Added for analytics
            "mode": mode,
            "ttl": ttl
        }
