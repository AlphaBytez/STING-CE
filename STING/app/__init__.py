...
from app.routes.demo_auth_routes import demo_auth_bp
...
def create_app(config=None):
    ...
    try:
        ...
        from app.middleware.maintenance_middleware import apply_maintenance_middleware
        apply_maintenance_middleware(flask_app)
    ...
    try:
        ...
        # Register demo authentication routes (public demo access)
        flask_app.register_blueprint(demo_auth_bp)
        logger.info("Demo authentication blueprint registered successfully")

        # Register admin management routes
        flask_app.register_blueprint(admin_bp)
        logger.info("Admin management blueprint registered successfully")
        
        # Register maintenance management routes
        try:
            from app.routes.maintenance_routes import maintenance_bp
            flask_app.register_blueprint(maintenance_bp)
            logger.info("🔧 Maintenance management blueprint registered successfully")
        except Exception as e:
            logger.warning(f"Maintenance blueprint registration failed: {e}")
            logger.info("Maintenance management features will be unavailable")
