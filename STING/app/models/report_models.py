...
class Report(Base):
    ...
    status_message = Column(String(255))  # Human-readable progress message
    current_stage = Column(String(50))  # Current processing stage: queued, collecting, generating, reviewing, formatting, saving
    ...
    # Relationships - lazy='joined' for efficient single-query loading
    template = relationship("ReportTemplate", back_populates="reports", lazy='joined')
    
    def to_dict(self, include_sensitive: bool = False, include_template: bool = True) -> Dict[str, Any]:
        """Convert to dictionary for API responses
        
        Args:
            include_sensitive: Include sensitive fields like parameters
            include_template: Include full template data (set False for list views to reduce payload)
        """
        data = {
            'id': self.id,
        ...
        # Include template data only when requested (skip for list views to reduce payload)
        if include_template and self.template:
            data['template'] = self.template.to_dict()
        else:
            # Include minimal template info for list displays
            data['template'] = {
                'name': self.template.name if self.template else None,
                'display_name': self.template.display_name if self.template else None
...
def get_queue_status(session: Session, queue_name: str = 'default') -> Dict[str, Any]:
    """Get current queue status and statistics - optimized single query"""
    from sqlalchemy import case
    
    # Single query with conditional aggregates for all counts
    result = session.query(
        func.count(case((Report.status == 'queued', 1))).label('pending'),
        func.count(case((Report.status == 'processing', 1))).label('processing'),
        func.avg(
            case(
                (
    ...
    return {
        'total_pending': result.pending or 0,
        'currently_processing': result.processing or 0,
        'avg_processing_time_seconds': result.avg_time or 0,
