...
async function testOllamaConnection() {
    try {
        ...
        const response = await makeRequest(`${OLLAMA_URL}/v1/models`);
        console.log('✅ Ollama is running');

        // OpenAI-compatible format uses 'data' instead of 'models'
        const models = response.data || [];
        console.log(`📋 Available models: ${models.length}`);

        models.forEach(model => {
            // OpenAI-compatible format uses 'id' instead of 'name'
