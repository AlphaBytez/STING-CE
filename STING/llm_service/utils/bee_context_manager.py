...
class BeeContextManager:
    ...
    def _apply_sliding_window(self, session: Dict) -> None:
        ...
        total_chars = sum(len(msg['content']) for msg in session['conversation_history'])
        estimated_tokens = total_chars // 4
        
        if estimated_tokens > self.max_context_length * 0.8:  # Leave 20% buffer
            # Remove oldest messages until we're under the limit
            while session['conversation_history'] and estimated_tokens > self.max_context_length * 0.8:
                removed = session['conversation_history'].pop(0)
...
if __name__ == "__main__":
    ...
    # Simulate response
    context_manager.update_assistant_response(session_id, "Hello! I'm Bee, your AI assistant.")
