...
from gateway.chat_api import router as chat_router
from gateway.native_proxy import native_proxy
...
async def generate_response(request: QueryRequest):
    ...
    # First, try the native LLM service via proxy
    logger.info(f"Attempting to use native LLM service for model: {model}")
    try:
        native_response = await native_proxy.generate(
        ...
        if native_response and 'response' in native_response:
            llm_response = native_response['response']
            logger.info("Successfully received response from native LLM service")
        ...
        logger.info(f"Native LLM service not available: {str(e)}")
    
    # If native service fails, try configured endpoints
