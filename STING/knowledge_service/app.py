...
## Key Features
- Honey Jars: Organized document repositories with semantic search
- AI-Powered Analysis: Use machine learning to analyze and understand your documents
- Bee Chat: Conversational AI assistant connected to your knowledge bases
- Document Processing: Upload and index documents for intelligent retrieval
- Report Generation: Create comprehensive reports from your knowledge base""",
...
STING honey jars organize documents by topic, project, or department.

## Step 2: Create Your Honey Jar
1. Navigate to Hive Manager
2. Click "Create New Honey Jar"
3. Name and describe your knowledge base
4. Set access permissions

## Step 3: Add Documents
Upload documents (PDF, Word, Markdown, etc.) and let STING index them for semantic search.""",
                                
                                "document_templates.json": """{"templates": [
{"name": "Project Documentation", "category": "technical", "fields": ["overview", "requirements", "architecture"]},
{"name": "Meeting Notes", "category": "collaboration", "fields": ["attendees", "agenda", "action_items"]},
{"name": "Policy Document", "category": "compliance", "fields": ["scope", "guidelines", "enforcement"]}
]}""",
...
- Ask Questions: Ask Bee about content in your honey jars
- Analyze Documents: Get summaries and key points from your uploads
- Generate Reports: Create comprehensive reports from your knowledge base
...
A: A honey jar is a knowledge repository that stores and organizes your documents with semantic search capabilities.

## Q: How does Bee chat work?
A: Bee uses AI to search your documents and provide intelligent answers based on your honey jar knowledge base.

## Q: Can I create custom honey jars?
A: Yes, STING supports custom honey jars for organizing documents by topic, project, or department.

## Q: How are documents processed?
A: STING uses semantic search and embeddings to make your documents intelligently searchable."""
                            }
...
async def ripen_honey_jar(
    ...
    This regenerates embeddings for all documents, fixing failed processing.
    Useful for demo data or after system issues.
    """
    repo = HoneyJarRepository(db)
    honey_jar = repo.get_honey_jar(honey_jar_id)
    ...
    doc_repo = DocumentRepository(db)
    documents = doc_repo.list_documents(honey_jar_id, limit=500)
    ...
    # Clear existing ChromaDB collection and recreate
    if chroma_available:
        collection_name = f"honey_jar_{str(honey_jar.id).replace('-', '_')}"
        try:
            ...
            try:
                chroma_client.delete_collection(collection_name)
                logger.info(f"Cleared existing ChromaDB collection: {collection_name}")
            except Exception:
                pass  # Collection might not exist
            
            # Create fresh collection
            semantic_search.get_or_create_collection(collection_name)
            logger.info(f"Created fresh ChromaDB collection for ripening: {collection_name}")
        except Exception as e:
            logger.error(f"Error managing ChromaDB collection: {e}")
    
    for document in documents:
        try:
            # Check if file exists
            if not document.file_path or not os.path.exists(document.file_path):
                # For demo documents, content might be in metadata
                extracted_text = ""
                if document.doc_metadata:
                    extracted_text = document.doc_metadata.get("extracted_text", "")
                    if not extracted_text and document.doc_metadata.get("content"):
                        extracted_text = document.doc_metadata.get("content", "")
                
                if not extracted_text:
                    logger.warning(f"No content found for document {document.filename}")
                    errors.append(f"{document.filename}: No content available")
                    failed_count += 1
                    continue
            else:
                # Read file content
            ...
            if not extracted_text:
                ...
                errors.append(f"{document.filename}: No text extracted")
                failed_count += 1
                continue
            ...
            if not chunks:
                ...
                errors.append(f"{document.filename}: No chunks created")
                failed_count += 1
                continue
            ...
            doc_repo.update_document(str(document.id), {
                ...
                "doc_metadata": {
                    **(document.doc_metadata or {}),
                    ...
                    "ripened_at": datetime.utcnow().isoformat()
                }
            })
            ...
            logger.error(f"Error ripening document {document.filename}: {e}")
    ...
    return {
        "message": f"Ripening complete: {processed_count} documents processed",
        "honey_jar_id": honey_jar_id,
...
@app.get("/honey-jars/{honey_jar_id}/documents/{document_id}/download")
async def download_document(
    ...
    """Download a document from a honey jar"""
    ...
    if not document or str(document.honey_jar_id) != honey_jar_id:
...
@app.get("/honey-jars/{honey_jar_id}/documents/{document_id}/preview")
async def preview_document(
    ...
    try:
        ...
        logger.error(f"Error reading document for preview: {e}")
