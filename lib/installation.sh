...
check_msting_path_conflicts() {
    ...
    # Run fix_permissions.sh after sudo is confirmed
    if [ "$is_sudo" = true ]; then
        log_message "Running permission fix script..."
        if [ -f "${INSTALL_DIR}/fix_permissions.sh" ]; then
            "${INSTALL_DIR}/fix_permissions.sh"
        elif [ -f "${SOURCE_DIR}/fix_permissions.sh" ]; then
            "${SOURCE_DIR}/fix_permissions.sh"
        else
            log_message "WARNING: fix_permissions.sh not found" "WARNING"
        fi
    fi
